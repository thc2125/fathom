...
class Seq2Seq(NeuralNetworkModel):
  ...
  def build_inference(self, xs):
    with self.G.as_default():
      ...
      if self.forward_only:
        self._outputs, self.losses = tf.contrib.legacy_seq2seq.model_with_buckets(
  ...
  def run(self, runstep=None, n_steps=1):
    ...
    log_dir = os.path.join('..','..','logs','seq2seq', 'tf_tools',
            'summary')
    summary_writer = tf.summary.FileWriter(log_dir, self.session.graph)
    while True:
      ...
      summary_writer.add_run_metadata(values, 'step_%d' % (current_step,),current_step)
